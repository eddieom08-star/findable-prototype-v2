...
export const CyclingPill = (): React.ReactElement => {
  ...
  return (
    <div className="flex items-center gap-3 mb-8 text-sm text-[color:var(--muted)] tracking-[0.02em]">
      ...
      <div
        className="relative inline-block align-middle h-[38px] overflow-hidden bg-[color:var(--orange)] text-white rounded-full px-[22px] min-w-[160px] shadow-[0_4px_14px_rgba(232,90,31,0.25)] font-medium text-sm tracking-[0.01em]"
        aria-live="polite"
        aria-label="cycling through trade types"
      >
        <div
          className="motion-safe:transition-transform motion-safe:duration-[550ms] motion-safe:ease-[cubic-bezier(0.7,0,0.3,1)]"
          style={{ transform: `translateY(-${index * ITEM_HEIGHT_PX}px)` }}
        >
          {PILL_TRADES.map((t) => (
            <div
              key={t}
              className="h-[38px] flex items-center justify-center whitespace-nowrap"
            >
              {t}
            </div>
          ))}
